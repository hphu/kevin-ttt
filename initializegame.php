...
//check to see if game in progress:
$resumegame = mysql_query("SELECT started, player1id FROM game WHERE player1id=($player) OR player2id=($player)");
$result = mysql_fetch_array($resumegame)['started'];
$game = mysql_fetch_array($resumegame)['player1id'];
if ($result==2){
	//create game entry
	$findgame = mysql_num_rows(mysql_query("SELECT started FROM game WHERE started=0 AND player1id!=($player)"));
	...
	if ($findgame ==0){
		...
		mysql_query("INSERT INTO game (started, gamestate, player1id, player2id, timestamp, justwent) VALUES (1, '000000000', ($opponentid), ($player), 0, 2)"); //game creator will go first, started =1
